...
export function utrAlertFields(details) {
  return [
    cell(details.team),
    // The slot cell carries the match id too ("M2 #07") — the approved WhatsApp
    // template has exactly five variables, and #07 alone could be any lobby.
    cell(details.slot, 12),
...
/* The message every channel except WhatsApp sends. WhatsApp is the odd one out: it
   ...
   which is also why the match name below is a text-channel extra rather than a sixth
   template field. */
